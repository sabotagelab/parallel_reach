...
private:
    ...
    geometry_msgs::PoseWithCovarianceStamped localized_pose_;
    nav_msgs::Path odom_path_, mpc_traj_;
...
MPCKinematicNode::MPCKinematicNode(ros::NodeHandle nh, ros::NodeHandle private_nh) {
    ...
    //Publishers and Subscribers
    odom_sub_ = nh.subscribe("/odom", 1, &MPCKinematicNode::odomCB, this);
...
void MPCKinematicNode::amclCB(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &amclMsg) {
    localized_pose_ = *amclMsg;
    if (_goal_received) {
        double car2goal_x = goal_pos_.x - amclMsg->pose.pose.position.x;
        double car2goal_y = goal_pos_.y - amclMsg->pose.pose.position.y;
...
void MPCKinematicNode::controlLoopCB(const ros::TimerEvent &) {
    if (_goal_received && !_goal_reached && _path_computed) //received goal & goal not reached
    {
        ...
        geometry_msgs::PoseWithCovarianceStamped localized_pose = localized_pose_;

        // Update system states: X=[x, y, psi, v]
        const double px = localized_pose.pose.pose.position.x; //pose: odom frame
        const double py = localized_pose.pose.pose.position.y;
        tf::Pose pose;
        tf::poseMsgToTF(localized_pose.pose.pose, pose);
