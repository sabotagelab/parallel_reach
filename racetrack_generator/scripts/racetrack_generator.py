...
class RacetrackGen:
    def __init__(self):	
        ...
        folder_name = rospy.get_param('path_folder_name', 'lab_track')
        ...
        self.OUTPUT_FILE_PATH = rospy.get_param('output_path',os.path.join(dirname,'generated_tracks/lab_track/'))
        ...
        self.max_speed = rospy.get_param('max_velocity', 3.0)
        self.min_speed = rospy.get_param('min_velocity', 0.9)
        self.CENTER_SMOOTH_VALUE =rospy.get_param('center_smooth_value', 0.75)
        self.CENTER_SCALING = rospy.get_param('center_scaling', 20)
        self.LEFT_SMOOTH_VALUE = rospy.get_param('left_smooth_value', 0.01)
        self.LEFT_SCALING = rospy.get_param('left_scaling', 500)
        self.RIGHT_SMOOTH_VALUE = rospy.get_param('right_smooth_value', 0.01)
        self.RIGHT_SCALING = rospy.get_param('right_scaling', 500)
        self.sort_dist_window_percent=3.0
        self.sort_angle_window_percent=1.5
        self.DEBUG_MODE = True
        self.REVERSE_MOVING_DIRECTION = True  #can select clockwise or anticlockwise direction
    ...
    def get_wall_point(self,last_wall_idx,wall_points,center_point,center_points_angles,idx):
        ...
        #         if desired_idx<SORT_ANGLES_WINDOW and last_wall_idx>wall_points.shape[0]-SORT_ANGLES_WINDOW:
        ...
        #             if self.DEBUG_MODE:
        #                 print angle_idx, sorted_delta_angle_idx[angle_idx],desired_idx,last_wall_idx
        #             angle_idx +=1
        #             desired_idx = sorted_distances_idx[sorted_delta_angle_idx[angle_idx]]
        #         else:
        #             break
    ...
    def racetrack_generator(self):
        ...
        second_derivative = self.get_interpolated_path(center_lane,smooth_value=self.CENTER_SMOOTH_VALUE,scale=self.CENTER_SCALING,derivative_order=2)
        ...
        vel_profile=np.interp(curvature, (curvature.min(), curvature.max()), (self.max_speed, self.min_speed))
        ...
        np.savetxt(self.OUTPUT_FILE_PATH + 'velocity_profile.csv', vel_profile, delimiter=",")
